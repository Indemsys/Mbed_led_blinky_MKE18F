...
// math.h required for floating point operations for baud rate calculation
  ...
  #define UART_CLOCK_FREQS    {    kCLOCK_ScgSysPllAsyncDiv2Clk, kCLOCK_ScgSysPllAsyncDiv2Clk, kCLOCK_ScgSysPllAsyncDiv2Clk  }

static uint32_t serial_irq_ids[FSL_FEATURE_SOC_UART_COUNT] = {0};
static uart_irq_handler irq_handler;

static LPUART_Type *const uart_addrs[] = LPUART_BASE_PTRS;      /* Array of UART peripheral base address. */

static clock_name_t const uart_clocks[] = UART_CLOCK_FREQS; /* Array of UART bus clock frequencies */
...
static inline void uart_irq(uint32_t transmit_empty, uint32_t receive_full, uint32_t index)
{
  ...
  {
    if (transmit_empty) irq_handler(serial_irq_ids[index], TxIrq);

    if (receive_full) irq_handler(serial_irq_ids[index], RxIrq);
  }
}
...
void uart1_irq(void)
{
  ...
  uart_irq((status_flags & LPUART_STAT_TDRE_MASK),(status_flags & LPUART_STAT_RDRF_MASK), 1);
}
...
void uart2_irq(void)
{
  ...
  uart_irq((status_flags & LPUART_STAT_TDRE_MASK),(status_flags & LPUART_STAT_RDRF_MASK), 2);
}
...
{
  IRQn_Type uart_rx_irqs[] = LPUART_RX_IRQS;
  IRQn_Type uart_tx_irqs[] = LPUART_TX_IRQS;
  ...
  if (enable)
  {
    ...
    NVIC_SetVector(uart_rx_irqs[obj->index], vector);
    NVIC_EnableIRQ(uart_rx_irqs[obj->index]);
    NVIC_SetVector(uart_tx_irqs[obj->index], vector);
    NVIC_EnableIRQ(uart_tx_irqs[obj->index]);
  ...
  else // disable
  {
    ...
    if (all_disabled) 
    {
      NVIC_DisableIRQ(uart_rx_irqs[obj->index]);
      NVIC_DisableIRQ(uart_tx_irqs[obj->index]);
    }
  }
}
...
int serial_getc(serial_t *obj)
{
  ...
  uint8_t data;
  data = LPUART_ReadByte(uart_addrs[obj->index]);
...
int serial_readable(serial_t *obj)
{
  ...
  if (status_flags & kLPUART_RxOverrunFlag) LPUART_ClearStatusFlags(uart_addrs[obj->index], kLPUART_RxOverrunFlag);
...
void serial_set_flow_control(serial_t *obj, FlowControl type, PinName rxflow, PinName txflow)
{
  ...
  case FlowControlRTS:
    ...
    uart_addrs[obj->index]->MODIR &= ~LPUART_MODIR_TXCTSE_MASK;
    uart_addrs[obj->index]->MODIR |= LPUART_MODIR_RXRTSE_MASK;
    break;
  ...
  case FlowControlRTSCTS:
    pinmap_pinout(rxflow, PinMap_UART_RTS);
    pinmap_pinout(txflow, PinMap_UART_CTS);
    uart_addrs[obj->index]->MODIR |= LPUART_MODIR_TXCTSE_MASK | LPUART_MODIR_RXRTSE_MASK;
    break;
